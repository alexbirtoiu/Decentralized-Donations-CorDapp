    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        when (command.value) {
            is Commands.Donate -> requireThat {
                ...
                val currency = iouToken.amount.token
                "This currency is not allowed" using (currency in currencies)

                val cashInputs = tx.inputsOfType<FungibleToken>()
                val validCashInputs = cashInputs.filter{it.amount.token == currency && it.holder == iouToken.lender}
            ...
            is Commands.Settle -> requireThat{
                ...
                "All token output states must be valid" using (outputTokens == validOutputTokens)
